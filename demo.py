...
def demo(path="data/demo.csv"):
    ...
    print("### PARETO FRONT ###")
    table = pareto_front(graph)
    if table:
        print("minimum latency path:")
        path_stats(graph, select_pareto_path(graph, table, 1, 0))
        print()
        print("minimum risk path:")
        path_stats(graph, select_pareto_path(graph, table, 0, 1))
        print()
        print("balanced path:")
        path_stats(graph, select_pareto_path(graph, table, 1, 10))
        print()
        print("minimum latency path with risk threshold = 5:")
        path_stats(graph, select_pareto_path(graph, table, None, 5))
        print()
        print("minimum risk path with latency threshold = 100:")
        path_stats(graph, select_pareto_path(graph, table, 100, None))
    print("####################\n\n")
